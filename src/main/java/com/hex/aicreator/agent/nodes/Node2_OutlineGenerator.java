...
public class Node2_OutlineGenerator implements NodeAction {
    ...
    public Map<String, Object> apply(OverAllState state) throws Exception {
        ...
        String subTitle = state.value(INPUT_SUB_TITLE).map(Object::toString).orElse("");

        String userDescription = state.value(INPUT_USER_DESCRIPTION).map(Object::toString).orElse(null);

        log.info("Node2_OutlineGenerator 开始执行: mainTitle={}, subTitle={}", mainTitle, subTitle);

        // 构建用户描述部分
        String descriptionSection = "";
        if (userDescription != null && !userDescription.trim().isEmpty()) {
            descriptionSection = PromptTemplate.USER_DESCRIPTION_POMPT.replace("{userDescription}", userDescription);
        }

        // 构建 prompt
        String prompt = PromptTemplate.AGENT2_OUTLINE_PROMPT.replace("{mainTitle}", mainTitle)
                .replace("{subTitle}", subTitle).replace("{descriptionSection}", descriptionSection);
        // 获取流式处理器
        Consumer<String> messageHandler = MessageHandlerContext.get();

        // 调用 LLM（流式输出）
        String content = stream(prompt, messageHandler, ArticleState.OutlineResult.class);

        // 解析结果
        ArticleState.OutlineResult outlineResult = GsonUtils.fromJson(content, ArticleState.OutlineResult.class);

        log.info("Node2_OutlineGenerator 执行完成: 生成了 {} 个章节", outlineResult.getSections().size());
    ...
    private String stream(String prompt, Consumer<String> messageHandler, Class clz) {
        ...
        response
                ...
                .doOnNext(output -> {
                    if (output instanceof StreamingOutput streamingOutput) {
                        ...
                        if (type == OutputType.AGENT_MODEL_STREAMING) {
                            ...
                            // 带前缀发送流式消息
                            messageHandler.accept(SseMessageTypeEnum.AGENT2_STREAMING.getStreamingPrefix() + text);
                ...
                // 错误处理, 记录日志
                .doOnError(error -> log.error("Node2_OutlineGenerator 流式调用失败", error))
